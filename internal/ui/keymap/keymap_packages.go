...
func DefaultPackages() Packages {
	return Packages{
		...
		Back: key.NewBinding(
			key.WithKeys("esc", "b", "P", "W"),
			key.WithHelp("esc/b/P/W", "back to runs"),
		),
		Help: key.NewBinding(
			key.WithKeys("?"),
...
func (p Packages) ShortHelp() []key.Binding {
	return []key.Binding{p.Up, p.Down, p.Open, p.Search, p.Refresh, p.AutoToggle, p.OpenBrowse, p.OpenReleases, p.Back, p.Help, p.Quit}
}
...
func (p Packages) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		...
		{p.OpenReleases, p.Refresh, p.AutoToggle, p.Help, p.Quit},
	}
}
